	...
	static partial class Program
	{
		...
		static void Peering2_WorkerTask(ZContext context, int i, string name) 
		{
			...
			using (var worker = ZSocket.Create(context, ZSocketType.REQ))
			{
				...
				worker.SendFrame(ZFrame.Create("READY"));
					...
					while (workers.Count > 0)
					{
						...
						using (incoming)
						{
							...
							if (reroutable == 1 && rnd.Next(5) == 0)
							{
								...
								using (var outgoing = new ZMessage())
								{
									outgoing.Add(ZFrame.Create(args[peer]));
									outgoing.Add(ZFrame.Create(string.Empty));
									outgoing.Add(ZFrame.Create(incoming[2].ReadString()));
							...
							else
							{
								...
								using (var outgoing = new ZMessage())
								{
									outgoing.Add(ZFrame.Create(peer));
									outgoing.Add(ZFrame.Create(string.Empty));
									outgoing.Add(ZFrame.Create(incoming[2].ReadString()));
