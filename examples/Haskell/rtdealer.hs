...
worker_task_a ctx = withSocket ctx XReq $ \worker -> do
...
worker_task_b ctx = withSocket ctx XReq $ \worker -> do
...
main = withContext 1 $ \context -> do
    withSocket context XRep $ \client -> do
        ...
        forkIO (worker_task_a context)
        forkIO (worker_task_b context)
        ...
        let func g _i = do
            ...
            if val > 0  then (send client (pack "A") [SndMore])
