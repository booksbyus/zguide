...
std::string kvmsg::to_string() {
  ...
  int is_text = s_is_text_data(body_);
  ss << "[" << std::setfill('0') << std::setw(3) << size() << "]";
  for (size_t char_nbr = 0; char_nbr < body_.size(); char_nbr++) {
    if (is_text)
      ss << (char)body_[char_nbr];
    else
      ss << std::setfill('0') << std::setw(2) << std::hex
         << (unsigned int)body_[char_nbr];
  }
...
//
//Main routine for runnint the basic test
//
//int main() {
